...
/**
 * This function is set as callback function to CURL to be called when the
 * response from the server is available; multiple calls for one response may
 * happen but that is behind this procedure, inside CURL.
 *  */
 ...
 * field it increments the next value depending on the gotn value, where gotn is
 * the number of items collected so far.
 *
 * @param pklist the list with pointers to the key-value mapping that is stored
 * in post_key structure;
 *
