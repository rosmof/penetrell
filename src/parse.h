...
static const char* href_start_marker =
    "javascript: __doPostBack\0";   // used to parse the next_href
static const char* event_validation_start_marker =
    "id=\x22__EVENTVALIDATION\x22\0";
...
typedef post_key* (*parse_func)(const char* raw_html, void* parse_object_type,
                                char* separator, size_t* result_size);

post_key* parse(parse_option popt, const char* html, void* ptype,
                char* separator, size_t* len);
 ...
 *  */
void get_value(const char* html, const char* start_criteria,
               const char* separator, const char* end, char* buffer,
               size_t bufflen);
 ...
 * is the next number
 *  */
post_key* parse_next_href(const char* html, void* pnext, char* separator,
                          size_t* len);

static std::map<parse_option, parse_func> parse_func_repository = {
    {parse_option::HREF_NEXT, parse_next_href}};
...
enum conversion_option {
    HREFNEXT_TO_POSTKEY = 1,
    EVENTVALID_TO_POSTKEY = 2,
    REQUESTDIGEST_TO_POSTKEY = 3
};
